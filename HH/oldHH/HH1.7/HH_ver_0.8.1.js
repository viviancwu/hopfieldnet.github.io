...
var userchoice = document.getElementById("t/m");
...
document.getElementById("t/m").addEventListener("change", VCsubmit); 
...
function VCcheck(event) {
	...
	if (userVCFlag.checked && !userVC2Flag.checked){
		...
		userselection.classList.remove("shown");
		userchoice.classList.add("hidden");
		userchoice.classList.remove("shown");
		...
		userselection.classList.add("shown");
		userselection.classList.remove("hidden");
		userchoice.classList.add("shown");
...
function VCsubmit(event) {
	...
	if(userVC2Flag.checked){
		...
		else if(userchoice.value=="membrane"){
			clampv2=-10;
			curd2=5;
			clamptime = 10;
			userclamptime.classList.add("hidden");
...
function vcrun() {
	...
	if(VC2.checked){
		loop1=curd/dt;
		loop2 = (clamptime+curd2)/dt;
		loop3 = (clamptime+2*curd2)/dt;
	}
