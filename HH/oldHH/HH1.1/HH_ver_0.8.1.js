...
function submit(event) {
  ...
  if (VC) {
	...
	seg = parseFloat(userseg.value);
	inputd = parseFloat(useriinputd.value);
...
function drun(){
	...
	for(var j=0; j<nSegments; j++){
		...
		currLeft = g_coupl*10^-9/A_seg*(vLeft[i]-vDend[i]);
		...
		if(j==nSegments-1){
			...
			vRight.push(vMem2);
			currRight = g_coupl*10^-9/A_seg*(vRight[i]-vDend[i]);
		}
		if(synLoc=j+1){
			vDend[i+1]=vDend[i]+deltaT/(tauM_seg*10^-3)*(R_seg*10^6*(currLeft+currRight-totalCurr2)+E_leak*10^-3-vDend[i]);
		}
		else{
			vDend[i+1] = vDend[i]+deltaT/(tauM_seg*10^-3)*(R_seg*10^6*(+currLeft+currRight)+E_leak*10^-3-vDend[i])
		}
	}
}
